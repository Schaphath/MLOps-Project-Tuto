...
def init_db_pool(retries: int = 5, delay: int = 3) -> Optional[pool.SimpleConnectionPool]:
    ...
    for i in range(retries):
        try:
            ...
            if i < retries - 1:
                logger.warning(f"Base de données inaccessible ({e}). Nouvelle tentative dans {delay}s... ({i+1}/{retries})")
                time.sleep(delay)
            else:
            ...
            try:
                ...
                with conn.cursor() as cursor:
                    ...
                    cursor.execute(query, (
                        input_dict["texture_worst"], input_dict["area_worst"], input_dict["smoothness_worst"],
                        input_dict["compactness_worst"], input_dict["concavity_worst"], input_dict["concave_points_worst"],
                        input_dict["symmetry_worst"], input_dict["fractal_dimension_worst"],
                        prediction_label, probability_pct
                    ))
                    conn.commit()
