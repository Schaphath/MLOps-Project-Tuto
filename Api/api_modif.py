...
def init_db_pool(retries: int = 5,
                 delay: int = 3) -> Optional[pool.SimpleConnectionPool]:
    ...
    for i in range(retries):
        try:
            ...
            logger.info(
                "Pool de connexions PostgreSQL initialisé avec succès.")
            return db_pool
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.critical(
            f"Erreur critique lors du chargement des modèles : {e}")
...
class PredictionInput(BaseModel):
    texture_worst: float = Field(...,
                                 gt=0,
                                 description="Valeur extrême de la texture cellulaire")
    area_worst: float = Field(...,
                              gt=0,
                              description="Valeur extrême de la surface cellulaire")
    smoothness_worst: float = Field(...,
                                    gt=0,
                                    description="Valeur extrême du lissé cellulaire")
    compactness_worst: float = Field(..., ge=0,
                                     description="Valeur extrême de la compacité")
    concavity_worst: float = Field(..., ge=0,
                                   description="Valeur extrême de la concavité")
    concave_points_worst: float = Field(..., ge=0,
                                        description="Valeur extrême des points concaves")
    symmetry_worst: float = Field(..., gt=0,
                                  description="Valeur extrême de la symétrie")
    fractal_dimension_worst: float = Field(
        ..., gt=0, description="Valeur extrême de la dimension fractale")
...
def health_check():
    ...
    if not models_ready:
        raise HTTPException(
            status_code=503,
            detail="Modèles prédictifs non chargés en mémoire.")
...
def predict(data: PredictionInput):
    ...
    try:
        ...
        prob_malignant = float(
            AppState.model.predict_proba(features_scaled)[0][1])
        ...
        if AppState.db_pool:
            ...
            try:
                ...
                if conn:
                    conn.rollback()
                logger.error(
                    f"Erreur d'écriture BDD (Transaction annulée) : {db_err}")
            finally:
                if conn:
                    AppState.db_pool.putconn(conn)
        else:
            logger.warning(
                "Sauvegarde impossible : Le pool de la base de données est inactif.")
